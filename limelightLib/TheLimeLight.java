...
public class TheLimeLight {
     ...
     */
    public TheLimeLight(String tableName) {
     ...
     */
    public TheLimeLight(NetworkTable table) {
        m_table = table;
        _hearBeat.startPeriodic(_hearBeatPeriod);
     ...
     * this is only distance from leftside of screen (not angle)
     * @return
     */
    ...
    /**
     * tx Horizontal Offset From Crosshair To Target (-27 degrees to 27 degrees)is altered
     *  by taking tx and subtracting half the width and then deviding by the width, this converts pixels 
     *  back to the -1 to 1 range where 1=100 percent away from crosshairs for the camera view
     * @return
     */
    public double getPercentRotationFromTarget() {
        NetworkTableEntry tx = m_table.getEntry("tx");
        double x = tx.getDouble(0.0);
        double factor = (x-170)/340;
        return factor;
    }
    ...
    public double getShorty() {
        NetworkTableEntry shorty = m_table.getEntry("tshort");
        double tshorty = shorty.getDouble(0.0);
        return tshorty;
    }

    public double getLongy() {
        NetworkTableEntry longy = m_table.getEntry("tlong");
        double tlongy = longy.getDouble(0.0);
        return tlongy;
    }

    public double getDistance()
    {
        //vertical angle of camera to target
        //double verticalOffset = Robot.theLimeLight.getLimeLight().getdegVerticalToTarget();
        
        //height of center of target
        //double targetHeight = RobotMap.(HATCH_TARGET_HEIGHT);
        
        //distance directly from target
        double directDistance = ((1.75) / (Math.tan(Math.toRadians(getdegVerticalToTarget()))));
        return directDistance;
    }

    public void returnValues()
    {
        SmartDashboard.putNumber("tx", getdegRotationToTarget());
        SmartDashboard.putNumber("ty", getdegVerticalToTarget());
        SmartDashboard.putNumber("tshorty", getShorty());
        SmartDashboard.putNumber("tlongy", getLongy());
        SmartDashboard.putNumber("skew", getSkew_Rotation());
        SmartDashboard.putNumber("longShortRatio", (getLongy() / getShorty()));
